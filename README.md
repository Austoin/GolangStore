...
- 商品详情页已支持真实数量选择与加入购物车
- 购物车已支持删除与勾选切换
- `/admin/products` 已支持新增商品与库存数量输入
- `/admin/orders` 当前已展示订单号、金额、状态、订单项数
- `/admin/products` 已支持新增商品，且可输入库存数量
- 后台商品管理已不只是展示页，而是可操作的管理入口
- 前台首页与商品详情页已重做为更现代的电商展厅布局
- `/admin/products` 已支持真实新增商品表单与库存数量输入
- 前台首页、商品详情、购物车、订单页都已完成现代化重排版
- 后台订单板块已补全订单号、金额、状态、订单项摘要展示
- 后台商品管理支持真实新增商品与库存数量输入
- 当前前后台页面与一键脚本已做完整联调验证
- 前台首页已支持加购成功/失败反馈
- 后台商品页与订单页已支持基础筛选入口
- 后台订单筛选已修复空订单号导致的异常
